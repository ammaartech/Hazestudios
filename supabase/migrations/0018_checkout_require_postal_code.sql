-- Require a postal code on the shipping address at checkout.
--
-- Qikink's create-order call fails outright without a postcode ("Shipping
-- address is missing a postcode"), and unlike address1/city/country it was
-- never enforced here — a shopper could check out with it blank, and the
-- order would sit unfulfillable until someone noticed at send time. The
-- checkout form and server action (0014) now require it too; this closes the
-- same gap at the function reachable without either of them.
