-- Size charts.
--
-- Two columns on `products` rather than a `product_size_charts` table: the
-- measurement set is per-product and sparse (a t-shirt lists chest and length,
-- a dress lists bust, waist, hip and length), and the whole chart is always
-- read and written as one block — never queried a measurement at a time. A
-- relational shape would mean ~36 rows per product for data with no query
-- surface of its own.
--
-- Shape of `size_chart`:
--   {
...
--     "rows":  [{ "size": "S", "values": { "chest": "38", "length": "27" } }],
--     "note":  "Model is 5'9\" and wears a size S."
--   }
--
-- Values are stored as strings in INCHES exactly as typed. Storing the operator's
-- own digits keeps "26-28" and "38.5" intact; the storefront converts to cm for
-- display. A numeric column could not hold a range.

alter table products
  add column if not exists size_chart_enabled boolean not null default false,
  add column if not exists size_chart         jsonb   not null default '{}'::jsonb;

-- ---------------------------------------------------------------------------
-- save_product — republished with the size chart carried through.
--
-- Copied wholesale from 0006 with only the size-chart lines added, which is the
-- same approach 0006 took to 0005: Postgres has no way to patch a function body,
-- so a versioned replacement is the honest unit of change.
-- ---------------------------------------------------------------------------
...
begin
  ...
  if v_id is null then
    insert into products (
      ...
      published_at, size_chart_enabled, size_chart
    ) values (
      v_title,
      v_handle,
