-- An address book per customer.
--
-- `customers.default_address` held exactly one address, so a shopper with a
-- home and an office had to retype one of them every time. This is the Swiggy /
-- Zomato model: addresses are saved as they are used, labelled, and offered
-- back at checkout as something to pick rather than something to type.
--
-- `default_address` is deliberately left in place and still written. It is what
-- the admin customer page reads, and orders placed before this migration only
-- have that. The book is additive; nothing depends on it existing yet.

create table if not exists customer_addresses (
  id           uuid primary key default gen_random_uuid(),
  customer_id  uuid not null references customers(id) on delete cascade,

  -- Free text rather than an enum. "Home" and "Work" cover most of it, but the
  -- moment someone types "Mum's place" an enum becomes a migration, and there
  -- is nothing here worth that.
  label        text not null default 'Home',

  -- Mirrors the shipping-address shape checkout already sends, key for key, so
  -- the jsonb on the order and the row here never need translating between.
  ...
  /*
   * What makes two addresses "the same one".
   *
   * Ordering to the same flat twice must not produce two entries, and the
   * comparison has to survive the ways people retype their own address —
   * trailing spaces, capitalisation. Name and label are excluded on purpose:
   * sending a parcel to the same door under a different recipient is still the
   * same door, and the row is updated rather than duplicated.
   *
   * Generated and stored, so the uniqueness is the database's to enforce
   * instead of every caller's to remember.
   */
  fingerprint  text generated always as (
    lower(btrim(address1)) || '|' ||
    lower(btrim(address2)) || '|' ||
    lower(btrim(city))     || '|' ||
...
create unique index if not exists customer_addresses_fingerprint_key
  on customer_addresses (customer_id, fingerprint);

-- At most one default per customer. Partial, so the many non-default rows do
...
-- RLS
-- ---------------------------------------------------------------------------
-- Scoped through `customers.user_id = auth.uid()`, exactly as 0009 scopes every
-- other customer-owned table. Unlike those, this one is writable by its owner:
-- managing your own addresses is the whole point. `with check` repeats the
-- predicate so a row can never be inserted against, or moved to, another
-- customer's id.
...
create policy customer_addresses_staff_all on customer_addresses
  for all to authenticated
  using (public.is_staff())
  with check (public.is_staff());

comment on table customer_addresses is
  'Saved delivery addresses per customer, labelled and de-duplicated by fingerprint. Written by place_order() as orders are placed.';

-- ---------------------------------------------------------------------------
-- place_order(): remember the address it just shipped to
-- ---------------------------------------------------------------------------
-- Otherwise identical to 0020.
