-- Require a phone number and a state/province on the shipping address.
--
-- Qikink's create-order call refuses an order missing either: phone
-- unconditionally (src/lib/qikink/map.ts raises "Order has no phone number,
-- which Qikink requires"), and province because their courier matching needs
-- it even though the checkout form left it optional. Both were reachable
-- blank through this function even after 0018 closed the same gap for
-- postal_code. The checkout form and server action now require them too;
-- this closes the gap at the function reachable without either.
